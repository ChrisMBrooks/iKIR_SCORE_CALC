rule call_hla_alleles_from_hla_imp:
    input:
        script = "scripts/call_hla_alleles_from_hla_imp.py",
        hla_file = "{0}".format(RAW_HLA_INPUT_FILENAME)
    params:
        output_dir = "outputs/{project}/hla",
        output_filename = "hla_allele_calls.csv",
        quality_threshold = CONFIG["rule_config"]["hla_imp_calling"]["quality_threshold"]
    output:
